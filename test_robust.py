...
# Create scenario that would reproduce the infinite loop
nodes = []
num_of_nodes = 10
neighbors = 3
...
print(f"Created {len(nodes)} nodes with {neighbors} neighbors each.")
print(f"Total stubs: {num_of_nodes * neighbors}")

stub_pairing(nodes, neighbors)

print("\nFinal network:")
for node in nodes:
    print(f"Node {node.node_id}: {len(node.neighbors)} neighbors = {node.neighbors}")
